...
class ConnectedSegments extends TrainbenchmarkQuery {
  ...
  class SemaphoreNeighbor extends TrainbenchmarkQuery {
    ...
    val antijoin = newLocal(Props(new HashLeftAntijoiner(production ! _, Vector(2, 5), Vector(1, 2))),"a")
  ...
  class RouteSensor extends TrainbenchmarkQuery{
    ...
    val antijoin = newLocal(Props(new HashLeftAntijoiner(production ! _, Vector(2, 3), Vector(0, 1))))
  ...
  class SwitchSensor extends TrainbenchmarkQuery {
    ...
    val antijoin = newLocal(Props(new HashLeftAntijoiner(production ! _, Vector(0), Vector(0))), "sw-antijoin")
